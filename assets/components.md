...
8. **骨架对 `.section` 内裸露的 `<p>`、`<ul>`、`<ol>`、`<li>`、`<h3>`、`<b>` 已经有基础排版**（行宽、间距、`overflow-wrap` 保护都有），不是所有内容都必须塞进下面 5–14 号具名组件才能显示。一段说明文字、一份不需要卡片强调的并列列表，直接写裸标签就够，不必为了"看起来是个组件"硬套某个卡片——那样反而会做成读者不想要的"卡片墙"。裸标签不算视觉锚点，第 5 条"一屏至多一个视觉锚点"约束的是大图和流程图，不限制这类纯文字排版。
...
什么时候用：一章内部有几条并列的实质要点需要立住时，放在该 section 末尾收束。

**不要用它做全文总结章。** 页面末尾专门加一章「读完这些，我能拿走什么」是明令禁止的写法（见 @references/writing-style.md 的「不写总结章」）——读者刚读完，不需要有人把说过的话再复述一遍。这个组件的位置是**章内**，收的是这一章自己的要点，不是全文的回顾。

每条要写实质判断，不要写成前文标题的换句话说。

```html
    <div class="takeaway">
      <ul><li>第一条实质要点</li><li>第二条实质要点</li></ul>
    </div>
```
